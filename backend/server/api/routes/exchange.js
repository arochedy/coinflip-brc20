...
module.exports = async () => {
  ...
  app.use(
    route.post("/deposit_arc", async (ctx) => {
      utils.logEvent(ctx.state.requestId, constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Validating the ARC Deposit payload`)

      const payload = ctx.checkPayload(ctx, "deposit")
      const { userId, transactionId, address } = payload

      utils.logEvent(ctx.state.requestId, constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Insert ARC deposit transaction`)
      await db.updateTransactionIdForAddress(userId, address, transactionId)
  ...
  app.use(
    route.post("/withdraw_arc", async (ctx) => {
      utils.logEvent(ctx.state.requestId, constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Validating the ARC Withdraw payload`)

      const payload = ctx.checkPayload(ctx, "withdraw")
      const { userId, transactionId, address, withdrawAddress, amount } = payload

      utils.logEvent(ctx.state.requestId, constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Insert ARC Withdraw transaction`)
      await db.updateTransactionIdForAddressOnWithdraw(userId, address, transactionId, withdrawAddress, amount)
