...
// --------------------------------------
// Get all pending transactions from btc_deposits table
const selectPendingTransactionsForWithdraws = async () => {
  const { rows: pendingTransactions } = await db.query("SELECT * FROM arc_withdraws WHERE status != $1 AND status != $2 AND transaction_id IS NOT NULL", ["confirmed", "expired"]);
  return pendingTransactions.length > 0 ? pendingTransactions : null;
}

// Update the status of a BTC deposit transaction to "analyzing"
const updateConfirmingTransactionForWithdraws = async (transactionId) => {
  await db.query("UPDATE arc_withdraws SET status = $2 WHERE transaction_id = $1", [transactionId, "analyzing"]);
  return;
}

// Update the status of a BTC deposit transaction to "invalid"
const updateAnalyzingTransactionInvalidForWithdraws = async (transactionId) => {
  await db.query("UPDATE arc_withdraws SET status = $2 WHERE transaction_id = $1", [transactionId, "expired"]);
  return;
}
...
async function updateTransactionIdForAddressOnWithdraw(userId, address, txId, withdrawAddress, amount) {
  await db.query(
    'UPDATE arc_withdraws SET transaction_id = $1 withdraw_address = $4 amount = $5 WHERE user_id = $2 AND deposit_address = $3',
    [txId, userId, address, withdrawAddress, amount]
  );
}
...
const getLatestUnspentDepositAddressForWithdraw = async (userId, address) => {
  const { rows: latestUnspentDepositAddress } = await db.query('SELECT * FROM arc_withdraws WHERE user_id = $1 AND deposit_address=$2', [userId, address]);
...
const insertDepositAddressForWithdraw = async (userId, address) => {
  // Insert the BTC deposit transaction into the database
  await db.query(`INSERT INTO arc_withdraws (user_id, deposit_address) VALUES ($1, $2);`, [userId, address])
}
...
const updateBalance = async (userId, txId, txBalance) => {
  ...
  for (let i = 0; i < pendingBalance.length; i++) {
    if (pendingBalance[i].transactionId === txId) {
  ...
  if (check == false) {
    await db.query("UPDATE user_balances SET balance = balance + $2 WHERE user_id = $1", [userId, txBalance])
  }
...
// Update the amount_received and set the status to "completed" for a BTC deposit transaction
const updateBtcAnalyzingTransactionForWithdraw = async (transactionId, status, amount, withdrawTransactionId="expired") => {
  const { rows: transactions } = await db.query("SELECT * FROM arc_withdraws WHERE transaction_id = $1", [transactionId]);
  if (transactions.length === 1) {
    if (status == "confimred") {
      const { rows: userBalance } = await db.query("SELECT * FROM  user_balances WHERE user_id = $1", [userId])
      if( userBalance[0].balance > amount ){
        ...
        try {
          await client.query("BEGIN")
          await db.query("UPDATE user_balances SET balance = balance - $2 WHERE user_id = $1", [userId, amount])
          await client.query("COMMIT")
          await db.query("UPDATE arc_withdraws SET status = $2, withdraw_transaction_id = $3 WHERE transaction_id = $1", [transactionId, status, "pending"]);
        } catch (e) {
          await client.query("ROLLBACK")
          await db.query("UPDATE arc_withdraws SET status = $2, withdraw_transaction_id = $3 WHERE transaction_id = $1", [transactionId, status, "expired"]);
          throw e
        } finally {
          client.release()
        }
      ...
      await db.query("UPDATE arc_withdraws SET withdraw_transaction_id = $2 WHERE transaction_id = $1", [transactionId, withdrawTransactionId]);
    }
    else
      await db.query("UPDATE arc_withdraws SET status = $2 WHERE transaction_id = $1", [transactionId, status]);
  }
  return false;
}
...
module.exports = {
  ...
  getLatestUnspentDepositAddressForWithdraw,
  updateTransactionIdForAddress,
  updateTransactionIdForAddressOnWithdraw,
  selectUserDetails,
  selectAchievements,
  selectLastLogin,
  updateUserDetails,
  ...
  selectPendingTransactionsForWithdraws,
  updateConfirmingTransactionForWithdraws,
  updateBtcAnalyzingTransactionForWithdraw,
  updateAnalyzingTransactionInvalidForWithdraws,
  selectUser,
  ...
  insertDepositAddressForWithdraw,
