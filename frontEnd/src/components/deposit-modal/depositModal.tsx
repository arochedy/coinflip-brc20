...
const DepositModal: FC<DepositModalProps> = ({ show, handleModal }) => {
	...
	const handleUnisatDepositTransaction = async () => {
		const depositAmount = amount.includes('.') ? parseFloat(amount) : parseInt(amount);
		let res = await GetExchangeAddress();
		let accountAddress = res.data.data;
		let walletAddresses = await window.unisat.getAccounts();
		...
		if (amount != '') {
			...
			if (unit == "SATS") {
				...
				try {
					// @ts-ignore
					let txid = await window.unisat.sendBitcoin(accountAddress, depositAmount);
					if (txid) {
				...
				try {
					...
					const isP2SHAddress = (address: string, network: bitcoin.Network): boolean => {
						try {
					...
					const utxoResult = await axios.get(`https://open-api.unisat.io/v1/indexer/address/${walletAddresses[0]}/brc20/ACD3/info`, {
						headers: {
							'Authorization': `Bearer ad863fd45eb93c01a31d364cb5f9165bfcdf4d84873155eec5ed39169a789016`
						}
					});
					...
					console.log(utxoResult);
					if (utxoResult.status == 200 && utxoResult.data && utxoResult.data.data && utxoResult.data.data.transferableInscriptions) {
						for (const info in utxoResult.data.data.transferableInscriptions) {
					...
					if (inscribeTxid == undefined) {
						...
						console.log("Built Inscription Data. Now sending btc for commit Tx to taproot address:", commitAddress);
						const commitTxId = await window.unisat.sendBitcoin(commitAddress, commitTxAmount);
						...
						const signedRevealPsbt = bitcoin.Psbt.fromHex(await window.unisat.signPsbt(revealPsbt.toHex(), {
							autoFinalized: false,
							toSignInputs: [
								{
									index: 0,
					...
					const getInscription = async (txid) => {
						return await retry(
							async (bail) => {
								let inscriptionResult = await window.unisat.getInscriptions();
								if (inscriptionResult.list && inscriptionResult.list.length > 0) {
					...
					const txid = await window.unisat.sendInscription(accountAddress, inscriptionId, {feeRate:70});
					///////////////////////////////////////////////////////////////////////////////////
	...
	const handleUnisatWithdrawTransaction = async () => {
		...
		if (amount != '') {
			console.log(unit);
			if (unit == "SATS") {
				enqueueSnackbar('Only ΛRC can be withdrawn', { variant: 'error', anchorOrigin: { horizontal: 'left', vertical: 'top' } });
				return;
			} else if (unit == "ΛRC") {
				// @ts-ignore
				...
				try {
					let txid = await window.unisat.sendBitcoin(accountAddress, 22000);
					if (txid) {
						// DepositBTC(false, txid);
						let result = await WithdrawARC(txid, accountAddress, walletAddresses[0], withdrawAmount);
						if (result?.status == 202) {
							enqueueSnackbar('Transaction Success', { variant: 'success', anchorOrigin: { horizontal: 'left', vertical: 'top' } })
							handleModal();
						} else {
