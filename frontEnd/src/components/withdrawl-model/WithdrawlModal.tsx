...
interface WithdrawlModal {
    show: boolean;
    handleModal: () => void;
}


const WithdrawlModal: FC<WithdrawlModal> = ({ show, handleModal }) => {
    ...
    const handleUnisatTransaction = async () => {
        ...
        if (amount != '') {
            // @ts-ignore
            if (amount < 1000) {
                enqueueSnackbar('Amount needs to be 1000 or more than that', { variant: 'error', anchorOrigin: { horizontal: 'left', vertical: 'top' } });
                return;
            }
            try {
                // @ts-ignore
