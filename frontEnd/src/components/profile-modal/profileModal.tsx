...
const ProfileModal: FC<ProfileModalProps> = ({ show, handleModal }) => {
	...
	const [activeBadges, setActiveBadge] = useState("winning")

	let winnerBadges = badges.filter(badge => badge.name !== "blank_badge" && !badge.name.includes("loss"));
	let looserBadges = badges.filter(badge => badge.name !== "blank_badge" && badge.name.includes("loss"));


	const fillLooserBadges = [];
	const fillWinnerBadges = [];

	if (badges.length && looserBadges.length < 9) {
		const looserBadgeEmptySlots = 9 - looserBadges.length;
		for (let i = 0; i < looserBadgeEmptySlots; i++) {
			fillLooserBadges.push({ id: i, name: "blank_badge", count: 0, desc: "" })
		}
		looserBadges = [...looserBadges, ...fillLooserBadges];
	}

	if (badges.length && winnerBadges.length < 9) {

		const winnerBadgesEmptySlots = 9 - winnerBadges.length;
		for (let i = 0; i < winnerBadgesEmptySlots; i++) {
			fillWinnerBadges.push({ id: i, name: "blank_badge", count: 0, desc: "" })
		}
		winnerBadges = [...winnerBadges, ...fillWinnerBadges];
	}
	...
	const handleBadgeToggle = () => {
		setActiveBadge(currentBadges => currentBadges == "winning" ? "loosing" : "winning")
	}
			...
			<div className="profile">
				...
				<div className="profile-value">
					...
					<div className="profile-value-item">
						...
						<span>
							{/* @ts-ignore */}
							{Math.round((parseFloat(data?.data.data.insights.totalAmountBet) + Number.EPSILON) * 100) / 100} ΛRC
						</span>
					</div>
					<div className="profile-value-item">
						<span>
					...
					<div className="profile-value-item">
						...
						<span>
							{/* @ts-ignore */}
							{Math.round((parseFloat(data?.data.data.insights.totalEarnings) + Number.EPSILON) * 100) / 100} ΛRC
						</span>
					</div>
					<div className="profile-value-item">
						<span>
						...
						<span>
							{/* @ts-ignore */}
							{Math.round((parseFloat(data?.data.data.insights.averageBetAmount) + Number.EPSILON) * 100) / 100} ΛRC
						</span>
					</div>
				...
				<div className="profile-other">
					<div className="history">
						...
						<div className="content">
							<ul className={`primary-list history-content`}>
					...
					<div className="badges">
						<div className="profile__badges-row">
							<div className="title">
								{activeBadges == "winning" ? "Winning badges" : "Fucking loser"}
							</div>
							<button onClick={handleBadgeToggle} className={`toggle___button ${activeBadges !== "winning" ? "toggle__button-active" : ""}`}>
								<span>
								</span>
							</button>
						</div>
						...
						<div className="content" style={{ display: activeBadges == "winning" ? "grid" : "none" }}>
							{
								winnerBadges?.map(badge => <div data-name={badge.name} data-id={badge.id} className="item" key={badge.name == "blank_badge" ? badge.id : badge.name} onClick={() => badge.name !== 'blank_badge' && handleBadgeModal(badge.name, badge.count, badge.desc)} >
									{badge.count.length && <span className="achievement_count">x{badge.count}</span>}
									{
										badge.name === 'blank_badge'
											? <img src={`/static/svgs/badges/blank_badge.svg`} className="blank" />
											: <img src={`/static/img/badges/${badge.name.toLowerCase()}.webp`} alt="" alt={badge.name} />
									}
								</div>)
							}
						</div>

						<div className="content" style={{ display: activeBadges == "loosing" ? "grid" : "none" }}>
							{
								looserBadges?.map(badge => <div className="item" key={badge.name == "blank_badge" ? badge.id : badge.name} onClick={() => badge.name !== 'blank_badge' && handleBadgeModal(badge.name, badge.count, badge.desc)} >
									{badge.count.length && <span className="achievement_count">x{badge.count}</span>}
									{
										badge.name === 'blank_badge'
											? <img src={`/static/svgs/badges/blank_badge.svg`} className="blank" />
											: <img src={`/static/img/badges/${badge.name.toLowerCase()}.webp`} alt="" alt={badge.name} />
										// <Image src={`/static/img/badges/${badge.name.toLowerCase()}.webp`} alt={badge.name} fill />
									}
								</div>)
							}
						</div>
					...
					<div className="deposite__widthdrawl">
						<div className="title">
							Tx History
						</div>
						<div className="content">
							<div className="theme__table-border">
